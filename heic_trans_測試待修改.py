...
def heic_trans_jpg():
    ...
    heif_dataset=search_heic_files(folder_path)

    def heif_gen():             # my_generator 是一个生成器函数，它会在每次迭代时产生一个值，并在下一次迭代时继续执行。
        for files_hg in heif_dataset:
            global filename
            filename=files_hg.split('.')[0] 
            heif_image=Image.open(f'{folder_path}/{files_hg}')
            jpg_dataset=jpg_dataset.append(files_hg)
            yield heif_image

    if jpg_dataset==[]:
        for heif_image in heif_gen():
            heif_image.save(f"{target_path}/{filename}.jpg",format="jpeg")
            c.set(f'轉檔{filename}')    
            
    else:
        im_rest_dataset=list(set(heif_dataset)-set(jpg_dataset))
        def heif_rest_gen():             # my_generator 是一个生成器函数，它会在每次迭代时产生一个值，并在下一次迭代时继续执行。
            for files_rest_hg in im_rest_dataset:
                global filename_r
                filename_r=files_rest_hg.split('.')[0]
                heif_image_r=Image.open(f'{folder_path}/{files_rest_hg}')
                yield heif_image_r

        for heif_image_r in heif_rest_gen():
          
            heif_image_r.save(f"{target_path}/{filename_r}.jpg",format="jpeg")
            c.set(f'轉檔{filename_r}')
